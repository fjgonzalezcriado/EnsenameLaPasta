...
#### HV-052 VIX como feature de régimen de mercado en el clasificador ML (sube/baja) ✅
- **Estado**: ✅ Completado · **Período**: 2026-08-31 · **Resultado**: ✅ Cumplido · **Tipo**: Evolutivo
- **Spec**: sin spec formal (evolutivo puntual a partir de una pregunta del usuario)
- **Resumen**: El usuario preguntó si el VIX (índice de volatilidad implícita del S&P 500, proxy de sentimiento/miedo de mercado) era relevante para sus posiciones y si merecía la pena que el ML lo tuviera en cuenta. Se amplía `MlDirectionClassifier` (HV-044/045): `FeatureCount` 12 → 14, con dos features nuevas — **nivel bruto del VIX** (la normalización MinMax del pipeline ya lo escala) y **variación diaria del VIX** — en el mismo índice temporal que las 12 features existentes (sin look-ahead adicional al ya inherente al diseño). `GetAlignedVixAsync` obtiene el histórico de `^VIX` vía el mismo `IMarketHistoryProvider` (Yahoo) para el mismo `range` del símbolo, y lo alinea **por día UTC** con la serie del símbolo (forward-fill sobre huecos de calendario entre bolsas, mismo criterio que HV-038); si el símbolo bajo análisis ya es `^VIX` o no hay datos del VIX (fuente caída, histórico corto…), degrada a features neutras (0) **sin lanzar** — mismo principio de "degrada sin romper" del resto del proyecto. 1 test nuevo (`ClassifyAsync_SinDatosDeVix_DegradaAFeatureNeutraSinRomper`); **191 verdes** (antes 190). Smoke real Yahoo: `AAPL 6M` → SDCA, `featureCount:14`, Comprar (Sube 92 %); `HY9H.F 6M` → FastTree, `featureCount:14` (VIX alineado entre Frankfurt y el calendario US del VIX); `^VIX 6M` como símbolo propio → `featureCount:14` con features de VIX a 0 (guard de auto-referencia), sigue prediciendo con normalidad. **Honestidad sin cambios**: accuracy/AUC siguen rondando 0,45-0,65 — el VIX es una feature de contexto adicional, no transforma el poder predictivo ya documentado en HV-044/045; indicador educativo, no asesoramiento financiero. Sin migración, sin cambios en Domain/persistencia.

#### HV-053 Agrupar posiciones abiertas por símbolo con precio medio ponderado (estilo bróker) ✅
- **Estado**: ✅ Completado · **Período**: 2026-08-25 · **Resultado**: ✅ Cumplido · **Tipo**: Mejora
- **Spec**: sin spec formal (mejora puntual solicitada directamente por el usuario)
- **Nota de numeración**: desarrollada en local como "HV-051"; renumerada a **HV-053** al integrar con el repo remoto, donde HV-051 corresponde a "Modo privacidad (ocultar importes económicos)".
- **Resumen**: En Trade Republic, comprar varios paquetes de la misma acción deja una única entrada en la cartera con el precio de compra medio ponderado. Se replica ese comportamiento **solo en la presentación** del dashboard (`dashboard.js`), sin tocar el modelo de datos: cada compra sigue siendo un `Trade` independiente en BD (preserva comisiones por orden HV-050, importación CSV HV-018/047 y desglose por periodo HV-049 tal cual). `renderOpenTrades` agrupa por símbolo cuando hay más de 1 lote abierto: fila resumen con precio de entrada = `Σ(entry×qty)/Σqty`, cantidad total, PnL/PnL% agregados y fecha de la compra más antigua; fila expandible (clic, icono ▸/▾) que despliega el detalle de cada lote con sus botones "Cerrar"/"Eliminar" de siempre (acción por Id, sin cambios). Símbolos con una sola compra se muestran igual que antes. Sin cambios en Domain/Application/Infrastructure/BD/tests (0 migraciones, 190 tests intactos). Verificado: `dotnet build` 0/0; smoke con BD SQLite temporal (3 lotes AAPL 100×10+120×5+90×15 → precio medio 98,33 y fecha de apertura la más antigua; 1 lote MSFT sin agrupar) confirmando la lógica de agrupación contra datos reales del endpoint `/api/dashboard/data`; sintaxis JS validada con `node --check`. Sin navegador disponible en esta sesión para captura visual — pendiente de verificación visual manual del usuario.
